...
public:
        ...
        :account(theAccount)
    {
        ...
        if(!mBlog)
            kError()<<"microblog is not a PlurkApiMicroBlog";
        Q_ASSERT(mBlog);
    }
...
PlurkApiSearchDialog::~PlurkApiSearchDialog()
{
    delete d;
}

void PlurkApiSearchDialog::createUi()
{
    kDebug();
...
void PlurkApiSearchDialog::fillSearchTypes()
{
    kDebug();
...
void PlurkApiSearchDialog::slotButtonClicked(int button)
{
    if(button == Ok) {
...
void PlurkApiSearchDialog::slotSearchTypeChanged(int )
{
    d->searchQuery->setFocus();
}

#include "plurkapisearchdialog.moc"
