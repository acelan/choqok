...
PlurkApiWhoisWidget::~PlurkApiWhoisWidget()
{
    kDebug();
    delete d;
}
...
void PlurkApiWhoisWidget::avatarFetched(const QString& remoteUrl, const QPixmap& pixmap)
{
    kDebug();
...
void PlurkApiWhoisWidget::avatarFetchError(const QString& remoteUrl, const QString& errMsg)
{
    kDebug();
    Q_UNUSED(errMsg);
    if( remoteUrl == d->currentPost.author.profileImageUrl ){
...
void PlurkApiWhoisWidget::updateHtml()
{
    kDebug();
    QString html;
...
void PlurkApiWhoisWidget::showForm()
{
    kDebug();
...
void PlurkApiWhoisWidget::show(QPoint pos)
{
    kDebug();
...
void PlurkApiWhoisWidget::checkAnchor( const QUrl url )
{
    kDebug();
...
void PlurkApiWhoisWidget::setupUi()
{
    kDebug();
...
void PlurkApiWhoisWidget::slotCancel()
{
    kDebug();
    if(d->waitFrame)
        d->waitFrame->deleteLater();
...
void PlurkApiWhoisWidget::setActionImages()
{
    d->imgActions.clear();
...
void PlurkApiWhoisWidget::slotFriendshipCreated(Choqok::Account* theAccount, const QString &username)
{
...
void PlurkApiWhoisWidget::slotFriendshipDestroyed(Choqok::Account* theAccount, const QString &username)
{
    if(theAccount == d->currentAccount && username == d->username){
        setActionImages();
        updateHtml();
...
// void PlurkApiWhoisWidget::slotUserBlocked(Choqok::Account* theAccount, const QString &username)
// {
//     if(theAccount == d->currentAccount && username == d->username){
...
#include "plurkapiwhoiswidget.moc"
