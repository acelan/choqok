...
const QRegExp PlurkSearch::m_rId("tag:search.plurk.com,[0-9]+:([0-9]+)");

PlurkSearch::PlurkSearch(QObject* parent): PlurkApiSearch(parent)
{
    kDebug();
    ...
    mSearchTypes[ToUser].first = i18nc( "Tweets are Plurk posts",  "Tweets To This User" );
    mSearchTypes[ToUser].second = true;

    mSearchTypes[FromUser].first = i18nc( "Tweets are Plurk posts", "Tweets From This User" );
    mSearchTypes[FromUser].second = true;

    mSearchTypes[ReferenceUser].first = i18nc( "Tweets are Plurk posts", "Tweets Including This Username" );
    mSearchTypes[ReferenceUser].second = true;

    mSearchTypes[ReferenceHashtag].first = i18nc( "Tweets are Plurk posts", "Tweets Including This Hashtag" );
    mSearchTypes[ReferenceHashtag].second = true;
}

void PlurkSearch::requestSearchResults(const SearchInfo &searchInfo,
...
void PlurkSearch::searchResultsReturned(KJob* job)
{
    kDebug();
    if( job == 0 ) {
...
QList< Choqok::Post* > PlurkSearch::parseAtom(const QByteArray& buffer)
{
    kDebug();
    ...
    while ( !node.isNull() ) {
        ...
        while ( !entryNode.isNull() ) {
            ...
            if ( elm.tagName() == "id" ) {
                // Fomatting example: "tag:search.plurk.com,2005:1235016836"
                ChoqokId id;
                if(m_rId.exactMatch(elm.text())) {
                    id = m_rId.cap(1);
                }
                /*                sscanf( qPrintable( elm.text() ),
                "tag:search.plurk.com,%*d:%d", &id);*/
                status->postId = id;
            ...
            } else if ( elm.tagName() == "plurk:source" ) {
...
QString PlurkSearch::optionCode(int option)
{
    return mI18nSearchCode[option];
}

PlurkSearch::~PlurkSearch()
{
}

KUrl PlurkSearch::buildUrl( QString query, int option, ChoqokId sinceStatusId,
                              uint count, uint page)
{
    kDebug();
    QString formattedQuery = mSearchCode[option] + query;
    KUrl url( "https://search.plurk.com/search.atom" );
