...
class PlurkApiTextEdit::Private
{
public:
    Private()
    :c(0)
...
PlurkApiTextEdit::PlurkApiTextEdit(uint charLimit, QWidget* parent)
...
PlurkApiTextEdit::~PlurkApiTextEdit()
{
    delete d;
}

void PlurkApiTextEdit::setCompleter(QCompleter *completer)
{
    if (d->c)
...
QCompleter *PlurkApiTextEdit::completer() const
{
    return d->c;
}

void PlurkApiTextEdit::insertCompletion(const QString& completion)
{
    if (d->c->widget() != this)
        return;
...
// QString PlurkApiTextEdit::textUnderCursor() const
// {
//     QTextCursor tc = textCursor();
//     tc.select(QTextCursor::WordUnderCursor);
...
void PlurkApiTextEdit::focusInEvent(QFocusEvent *e)
{
    if (d->c)
        d->c->setWidget(this);
...
void PlurkApiTextEdit::keyPressEvent(QKeyEvent *e)
{
